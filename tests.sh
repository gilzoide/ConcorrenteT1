...
do
	...
	for((i=0 ;i<10;i++))
	do
		(/usr/bin/time -f "%e" ./build/main < matrizes/$test_file) 2>> tmp-$test_file 1> result-$test_file
		total_time_single=$(python -c "print $total_time_single + $(tail -n 1 tmp-$test_file)")

		(/usr/bin/time -f "%e" ./build/main --multi < matrizes/$test_file) 2>> tmp-$test_file 1> result-$test_file
	...
	python achaDesvio.py tmp-$test_file
	echo
